...
function sama_woocommerce_shortcode_products_query() {
	/*
	 * grid 			= 30
	 * list = list1 	= 31
     * list2 			= 32
	 ...
	 * 3col				= 3
	*/
	global $majesty_options, $woocommerce_loop;
	$woocommerce_loop['columns'] = absint($woocommerce_loop['columns']);
	if ( $woocommerce_loop['columns']  == 37 ) {
		$layout = 'grid';
	} elseif ( $woocommerce_loop['columns']  == 31 ) {
		$layout = 'list';
	} elseif ( $woocommerce_loop['columns']  == 32 ) {
		$layout = 'list2';
	} elseif ( $woocommerce_loop['columns']  == 33 ) {
		$layout = 'masonry';
	} elseif ( $woocommerce_loop['columns']  == 34 ) {
		$layout = 'masonryfullwidth';
	} elseif ( $woocommerce_loop['columns']  == 35 ) {
		$layout = 'gridfullwidth';
	} elseif ( $woocommerce_loop['columns']  == 36 ) {
		$layout = 'grid4col';
	...
	if( in_array( $layout, $shortcode_layous ) ) {
		$majesty_options['shortcode_products_query'] = $layout;
...
function sama_woocommerce_short_description_wrap( $content ) {
	return '<div class="desc-content">'. wp_kses_post($content) .'</div>'; 
}
/*
...
function sama_woocommerce_output_before_related_products() {
	...
	$related = wc_get_related_products($product->id);
	if ( sizeof( $related ) == 0 ) {
		return;
	} else {
...
function sama_woocommerce_image_dimensions() {
	...
	/**
	 * Control number of columns for products category in shop archive page
	 *
	 * @ since 1.4.3
	 */
	if (!function_exists('sama_woo_cat_loop_columns')) {
		function sama_woo_cat_loop_columns() {
			global $majesty_options;
			return absint( $majesty_options['woo_cat_per_row'] );
		}
	}
	
	if ( !function_exists('sama_add_shop_categories_for_shop_archive') ) {
		function sama_add_shop_categories_for_shop_archive() {
			global $majesty_options;
			if( ! ( '' === get_option( 'woocommerce_shop_page_display') ) ) {
				$majesty_options['default_woocommerce_shop_page_display'] = get_option( 'woocommerce_shop_page_display');
				$majesty_options['woo_display_cat_loop'] = true;				
			}
			if( ! ( '' === get_option( 'woocommerce_category_archive_display') ) ) {
				$majesty_options['default_woocommerce_archive_page_display'] = get_option( 'woocommerce_category_archive_display');
				$majesty_options['woo_display_cat_loop'] = true;
			}
			if ( $majesty_options['woo_display_cat_loop'] == true ) {
				add_filter('loop_shop_columns', 'sama_woo_cat_loop_columns');
	...
	if (!function_exists('sama_shop_categories_woo_default')) {
		function sama_shop_categories_woo_default() {
			global $majesty_options;
			update_option( 'woocommerce_shop_page_display', $majesty_options['default_woocommerce_shop_page_display'] );
			update_option( 'woocommerce_category_archive_display', $majesty_options['default_woocommerce_archive_page_display'] );
		}
	}	
}